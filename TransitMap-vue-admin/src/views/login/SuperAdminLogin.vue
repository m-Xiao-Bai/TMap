<template>
  <div class="login-container super-admin-login">
    <div class="login-bg">
      <div class="metro-lines"></div>
    </div>
    <div class="login-card">
      <div class="login-header">
        <div class="logo-icon super-icon">
          <el-icon :size="40"><Finished /></el-icon>
        </div>
        <h2>城市轨道交通管理系统</h2>
        <p class="subtitle">超级管理员登录</p>
      </div>
...
import { Lock, Message, Phone, Key, Finished } from '@element-plus/icons-vue'
import { superAdminLogin, getCaptchaImage } from '@/api/user'
...
.super-admin-login {
  background: linear-gradient(135deg, #0c1220 0%, #1b2838 50%, #0e4d44 100%);
}
...
.metro-lines {
  ...
  left: -50%;
  background:
    radial-gradient(circle at 30% 70%, rgba(0, 210, 180, 0.1) 0%, transparent 50%),
...
.super-icon {
  background: linear-gradient(135deg, #00d2b4, #0096c8);
...
.super-btn:hover {
  background: linear-gradient(135deg, #66e8d8, #66c8e8);
}
