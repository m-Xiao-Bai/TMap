<template>
  <div class="login-container root-admin-login">
    <div class="login-bg">
      <div class="metro-lines"></div>
    </div>
    <div class="login-card">
      <div class="login-header">
        <div class="logo-icon root-icon">
          <el-icon :size="40"><TrophyBase /></el-icon>
        </div>
        <h2>城市轨道交通管理系统</h2>
        <p class="subtitle">最高级管理员登录</p>
      </div>
...
import { Lock, Message, Phone, Key, TrophyBase } from '@element-plus/icons-vue'
import { rootAdminLogin, getCaptchaImage } from '@/api/user'
...
const handleLogin = async () => {
  ...
  loading.value = true
  try {
    const res = await rootAdminLogin({
...
.metro-lines {
  ...
  left: -50%;
  background:
    radial-gradient(circle at 25% 75%, rgba(200, 100, 255, 0.1) 0%, transparent 50%),
    radial-gradient(circle at 75% 25%, rgba(255, 180, 50, 0.1) 0%, transparent 50%);
...
.root-icon {
  background: linear-gradient(135deg, #c864ff, #ffb432);
  box-shadow: 0 4px 15px rgba(200, 100, 255, 0.4);
}
...
.root-btn:hover {
  background: linear-gradient(135deg, #dca0ff, #ffd080);
}
