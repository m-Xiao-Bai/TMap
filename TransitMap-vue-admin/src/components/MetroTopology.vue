      ...
      <g ref="zoomGroupRef">
        ...
        <g v-for="line in renderedLines" :key="'line-' + line.name"
          ...
          <path
            ...
            fill="none"
            :stroke="line.color"
            :stroke-width="selectedLine === line.name ? 10 : 6"
            :stroke-opacity="selectedLine === line.name ? 0.2 : 0.1"
          ...
          <path
            ...
            fill="none"
            :stroke="line.color"
            :stroke-width="selectedLine === line.name ? 5.5 : 3.5"
        ...
        <g v-for="station in layoutStations" :key="'station-' + station._id"
          ...
          <circle
            v-else
            :cx="station._x" :cy="station._y"
            :r="pathRoute.includes(station._id) ? 8 : 6"
            :fill="isPathEndpoint(station._id) ? '#ff6b35' : '#fff'"
            :stroke="isPathEndpoint(station._id) ? '#ff6b35' : '#303133'"
            :stroke-width="pathRoute.includes(station._id) ? 2.5 : 2"
...
const svgW = 1400, svgH = 900, PAD = 80
...
const labeledStations = computed(() => {
  ...
  for (const s of sList) {
    ...
    const charW = s.isTransfer === 1 ? 9 : 7
    const lineH = 10
...
.station-dot:hover { r: 6; stroke-width: 2; }
.station-dot.transfer:hover { r: 8; stroke-width: 2.5; }
...
/* ── 站名 ── */
.station-label {
  font-family: 'Cascadia Code', 'Fira Code', 'Consolas', 'Noto Sans SC', monospace;
  font-size: 8px; fill: #5a5a6e; font-weight: 600; pointer-events: none;
  paint-order: stroke fill; stroke: #f8f9fb; stroke-width: 2.5px; stroke-linejoin: round;
  letter-spacing: 0.3px;
}
.station-label.transfer { font-size: 9px; font-weight: 800; fill: #1a1a2e; stroke-width: 2.5px; }
...
.metro-tooltip-card {
  ...
  white-space: nowrap; font-family: 'Cascadia Code', 'Fira Code', 'Consolas', 'Noto Sans SC', monospace;
}
.tooltip-name { font-size: 12px; font-weight: 700; margin-bottom: 1px; }
.tooltip-en { font-size: 10px; opacity: 0.6; margin-bottom: 3px; }
.tooltip-lines { font-size: 10px; opacity: 0.75; }
.tooltip-transfer { font-size: 9px; margin-top: 3px; padding: 1px 5px; background: rgba(255, 200, 50, 0.2); color: #ffc832; border-radius: 3px; display: inline-block; }
</style>
