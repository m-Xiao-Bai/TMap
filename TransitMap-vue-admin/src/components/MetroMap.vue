...
function renderMap() {
  ...
  if (props.lines.length > 0) {
    for (const line of props.lines) {
      ...
      L.polyline(coords, { color: line.color || '#409EFF', weight: 8, opacity: 0.12, lineJoin: 'round', lineCap: 'round' }).addTo(lineGroup)
      L.polyline(coords, { color: line.color || '#409EFF', weight: 4, opacity: 0.9, lineJoin: 'round', lineCap: 'round' }).addTo(lineGroup)
...
function drawStationMarker(station, isTransfer) {
  ...
  let marker
  if (isTransfer) {
    marker = L.circleMarker([lat, lng], { radius: 6, fillColor: '#ffffff', color: '#303133', weight: 2.5, opacity: 1, fillOpacity: 1 })
  } else {
    marker = L.circleMarker([lat, lng], { radius: 4, fillColor: '#ffffff', color, weight: 2, opacity: 1, fillOpacity: 1 })
  }

  const tooltipText = station.stationName
  marker.bindTooltip(tooltipText, {
    permanent: isTransfer,
    direction: 'bottom',
    offset: [0, isTransfer ? 10 : 6],
...
.metro-station-tooltip {
  font-size: 10px !important; font-weight: 500 !important; padding: 2px 7px !important;
  border-radius: 4px !important; background: rgba(48, 49, 51, 0.78) !important; color: #fff !important;
  border: none !important; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.1) !important;
  font-family: 'Cascadia Code', 'Fira Code', 'Consolas', 'Noto Sans SC', monospace !important;
  letter-spacing: 0.3px;
}
.metro-station-tooltip::before { border-bottom-color: rgba(48, 49, 51, 0.78) !important; }
.metro-transfer-tooltip {
  font-size: 10px !important; font-weight: 700 !important; padding: 2px 8px !important;
  border-radius: 4px !important; background: rgba(26, 26, 46, 0.85) !important; color: #fff !important;
  border: none !important; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.15) !important;
  font-family: 'Cascadia Code', 'Fira Code', 'Consolas', 'Noto Sans SC', monospace !important;
  letter-spacing: 0.3px;
}
.metro-transfer-tooltip::before { border-bottom-color: rgba(26, 26, 46, 0.85) !important; }
