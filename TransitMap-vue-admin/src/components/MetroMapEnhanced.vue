...
defineExpose({ clearRoute: clearRouteOnly, findRoute: findAndShowRoute })
...
function highlightRoute(route) {
  ...
  lineGroup.eachLayer(layer => {
    if (layer._lineName) {
      layer.setStyle({ opacity: layer._isGlow ? 0.04 : 0.12, weight: layer._isGlow ? 8 : 3 })
    }
  })
  ...
  for (const [id, marker] of Object.entries(stationMarkers)) {
    ...
    if (!onRoute) {
      marker.setStyle({ opacity: 0.12, fillOpacity: 0.12, radius: 3 })
      const tooltip = marker.getTooltip()
  ...
  if (coords.length >= 2) {
    L.polyline(coords, {
      color: '#ff6b35', weight: 14, opacity: 0.25,
      lineJoin: 'round', lineCap: 'round',
    }).addTo(routeGroup)
    L.polyline(coords, {
      color: '#ff6b35', weight: 6, opacity: 0.95,
      lineJoin: 'round', lineCap: 'round',
      dashArray: '12 6',
  ...
  for (const id of route) {
    ...
    marker.setStyle({
      fillColor: isEndpoint ? '#ff6b35' : '#fff',
      color: '#ff6b35',
