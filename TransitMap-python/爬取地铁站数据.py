...
爬取地铁站数据（完整版）
========================
从 OpenStreetMap Overpass API 爬取全国主要城市的地铁站数据，
支持单城市和批量模式，生成 Excel 文件。

用法:
  python 爬取地铁站数据.py                    # 批量爬取所有城市
  python 爬取地铁站数据.py 南昌市             # 爬取单个城市
  python 爬取地铁站数据.py 南昌市 广州市      # 爬取多个指定城市
...
def main():
    ...
    print(f'开始爬取 {len(cities)} 个城市地铁站数据...', flush=True)
    ...
    if all_stations:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        all_path = os.path.join(OUTPUT_DIR, f'地铁站数据_全部_{timestamp}.xlsx')
        save_to_excel(all_stations, all_path)
        print(f'\n合并文件: {all_path}')
